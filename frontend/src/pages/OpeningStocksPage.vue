<template>
  <div class="erp-page">
    <div class="erp-toolbar">
      ...
      <el-button
        type="primary"
        :loading="dialogLoading && !editingOpeningStock"
        @click="openDialog()"
      >
    ...
    <el-card>
      <el-table
        :data="openingStockStore.pagedOpeningStocks.items"
        v-loading="openingStockStore.loading"
        stripe
      >
...
import type {
  CreateOpeningStock,
  UpdateOpeningStock,
  CreateOpeningStockDetail,
} from "@/services/opening-stocks";
...
const handleSubmit = async () => {
  ...
  const hasInvalidItem =
    form.value.openingStockDetails.length === 0 ||
    form.value.openingStockDetails.some((detail) => !detail.itemId);

  if (hasInvalidItem) {
    ElMessage.warning(
      "Add at least one valid item before saving opening stock",
    );
    return;
  }
