<template>
    <div class="erp-page">
        <div class="erp-toolbar">
            ...
            />
            <el-button type="primary" :loading="dialogLoading && !editingPurchase" @click="openDialog()">
        ...
        <el-card>
            <el-table :data="purchaseStore.pagedPurchases.items" v-loading="purchaseStore.loading" stripe>
                <el-table-column prop="id" label="ID" width="80" />
                <el-table-column prop="invoiceNumber" label="Invoice Number" width="180" />
                <el-table-column prop="purchaseDate" label="Date" width="150">
            ...
            <el-form :model="form" :rules="rules" ref="formRef" label-width="150px" class="erp-dialog-form" v-loading="dialogLoading">
                <el-row :gutter="20">
                    <el-col :span="12">
                        <el-form-item label="Supplier" prop="supplierId">
                            <el-select v-model="form.supplierId" placeholder="Select supplier" style="width: 100%">
                                <el-option
                ...
                <el-table :data="form.purchaseDetails" border style="width: 100%">
                    ...
                    <el-table-column label="Quantity" width="120">
                        <template #default="{ row }">
                            <el-input-number v-model="row.quantity" :min="1" :step="1" style="width: 100%" @change="calculateTotals" />
                        </template>
                    </el-table-column>
                    <el-table-column label="Purchase Rate" width="140">
                        <template #default="{ row }">
                            <el-input-number v-model="row.purchaseRate" :min="0" :precision="2" :step="0.01" style="width: 100%" @change="calculateTotals" />
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column label="Discount" width="120">
                        <template #default="{ row }">
                            <el-input-number v-model="row.discount" :min="0" :precision="2" :step="0.01" style="width: 100%" @change="calculateTotals" />
                        </template>
                    </el-table-column>
                    <el-table-column label="Tax %" width="100">
                        <template #default="{ row }">
                            <el-input-number v-model="row.taxPercentage" :min="0" :max="100" :precision="2" :step="0.01" style="width: 100%" @change="calculateTotals" />
                        </template>
                    </el-table-column>
...
import type { CreatePurchase, UpdatePurchase, CreatePurchaseDetail } from "@/services/purchases";
...
const rules: FormRules = {
    supplierId: [{ required: true, message: "Supplier is required", trigger: "change" }],
    purchaseDate: [{ required: true, message: "Date is required", trigger: "change" }],
    invoiceNumber: [{ required: true, message: "Invoice number is required", trigger: "blur" }]
};

const totalAmount = computed(() => {
    return form.value.purchaseDetails.reduce((sum, detail) => sum + calculateLineTotal(detail), 0);
});
...
const handleSubmit = async () => {
    ...
    const hasInvalidItem = form.value.purchaseDetails.length === 0
        || form.value.purchaseDetails.some(detail => !detail.itemId || detail.quantity <= 0);
...
const handleDelete = async (id: number) => {
    try {
        await ElMessageBox.confirm("Are you sure you want to delete this purchase?", "Confirm", {
            type: "warning"
        });
