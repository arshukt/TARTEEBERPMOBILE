<template>
  <div class="erp-page erp-card-stack">
    <h1 class="erp-page-heading">Supplier Outstanding</h1>
    <el-card>
      <el-table :data="reportData" v-loading="loading" style="width: 100%">
        <el-table-column prop="supplierName" label="Supplier" width="150" />
        <el-table-column
          prop="totalOutstanding"
          label="Outstanding"
          width="120"
        >
...
import {
  reportService,
  type SupplierOutstandingReportItem,
} from "@/services/reports";

const loading = ref(false);
const reportData = ref<SupplierOutstandingReportItem[]>([]);
...
const fetchReport = async () => {
  loading.value = true;
  try {
    const response = await reportService.getSupplierOutstanding();
