...
export const reportService = {
  ...
  async getPurchases(
    startDate?: string,
    endDate?: string,
  ): Promise<ApiResponse<PurchasesReportItem[]>> {
    const params = new URLSearchParams();
    if (startDate) params.append("startDate", startDate);
    if (endDate) params.append("endDate", endDate);
  ...
  async getCustomerOutstanding(): Promise<
    ApiResponse<CustomerOutstandingReportItem[]>
  > {
    const response = await api.get("/reports/customer-outstanding");
    return response.data;
  },

  async getSupplierOutstanding(): Promise<
    ApiResponse<SupplierOutstandingReportItem[]>
  > {
